            ...
            <div class="col-12">
                <div class="card mb-4">
                    <div class="card-header pb-0 d-flex justify-content-between align-items-center">
                        <h6>{{ $page->title }}</h6>
                        <a href="{{ url('kriteria9/input') }}" class="btn btn-sm btn-success">
                            {{ __('input') }}
                        </a>
                    </div>
                ...
                columns: [{
                    ...
                    },
                    {
                        ...
                        render: function(data, type, row, meta) {
                            ...
                            let detailBtn =
                                `<button class="btn btn-info btn-xs" onclick="modalAction('${base_url}/${id}/show')">{{ __('detail') }}</button>`;
                            // Jika status submit, disable tombol edit
                            let editBtn = '';
                            if (status === 'submit') {
                                editBtn =
                                    `<a class="btn btn-secondary btn-xs disabled" href="#">{{ __('edit') }}</a>`;
                            } else {
                                editBtn =
                                    `<a class="btn btn-warning btn-xs" href="${base_url}/${id}/edit">{{ __('edit') }}</a>`;
                            }
                            
                            let deleteBtn =
                                `<button class="btn btn-danger btn-xs" onclick="modalAction('${base_url}/${id}/delete')">{{ __('delete') }}</button>`;
